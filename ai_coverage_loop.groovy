...
def run(script, env, params, sha1, lcovParser, CONTEXT_FILES) {
    ...
    // -------------------------------------------------------------------
    // --- RAG INDEXING (Must run before the loop starts) ---
    // Note: This block was outside the loop in your original Jenkinsfile, 
    // but is included here for completeness if you decide to move it back.
    script.echo "Indexing codebase for Semantic Search (RAG)..."
    def contextFilesString = CONTEXT_FILES.join(' ')
    ...
    while (iteration < maxIterations) {
        ...
        script.withCredentials([script.string(credentialsId: 'GEMINI_API_KEY_SECRET', variable: 'GEMINI_API_KEY')]) {
            script.echo "Analyzing coverage files, iteration ${iteration}..."
            script.sh """
            ./venv/bin/python3 ${env.PROMPT_SCRIPT} --prompt-file '${promptFilePath}' '${env.COVERAGE_INFO_FILE}' '${outputPath}'
            """ 
        }
